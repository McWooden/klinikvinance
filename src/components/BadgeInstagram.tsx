import React from "react";

interface BadgeInstagramProps {
  width?: number | string;
  height?: number | string;
  className?: string;
}

const BadgeInstagram: React.FC<BadgeInstagramProps> = ({
  width = 33,
  height = 33,
  className,
}) => {
  return (
    <svg
      width={width}
      height={height}
      viewBox="0 0 33 33"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
    >
      <rect
        x="0.5"
        y="0.677246"
        width="32"
        height="32"
        fill="url(#pattern0_2910_2194)"
      />
      <defs>
        <pattern
          id="pattern0_2910_2194"
          patternContentUnits="objectBoundingBox"
          width="1"
          height="1"
        >
          <use xlinkHref="#image0_2910_2194" transform="scale(0.01)" />
        </pattern>
        <image
          id="image0_2910_2194"
          width="100"
          height="100"
          preserveAspectRatio="none"
          xlinkHref="data:image/png;base64,"
        />
      </defs>
    </svg>
  );
};

export default BadgeInstagram;
